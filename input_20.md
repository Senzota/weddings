...
Status: implemented and tested locally. **Not committed, not pushed, no
migration needed** (this phase adds no schema change at all).

### Files changed

Only `config/themes.js` — every other file in this phase's scope
(`input_20.md`, `PROMPT_REPORT.md`) is documentation, not code.

Each of the three existing `AVAILABLE_THEMES` entries gained two new,
optional fields:
- `swatchColors`: 4 hex colors each, hand-picked from that theme's own
  `public/themes/<slug>/theme.css` custom properties (not generated by any
  parser, build step, or image process — read directly from the CSS file and
  copied in):
  - `botanical-bloom`: `#f6f2e7` (`--bb-cream`), `#9f5b4c` (`--bb-rose`),
    `#7f8f5f` (`--bb-sage`), `#3a3128` (`--bb-ink`).
  - `lavender-romance`: `#f7efe3` (`--bb-cream`), `#cbb8d0` (`--bb-lavender`),
    `#755062` (`--bb-rose`, mauve-dark), `#fffdf9` (`--bb-ivory`).
  - `lady-gianna`: `#f9ece6` (`--lg-blush`), `#d98a96` (`--lg-rose`),
    `#c6a15b` (`--lg-gold`), `#a3b48c` (`--lg-sage`).
- `tagline`: one short, customer-facing sentence per theme, written to match
  each theme's existing tone (Botanical Bloom's earthy/garden language,
  Lavender Romance's "wedding atelier" romantic framing, Lady Gianna's
  festive/celebratory framing).

No slug, `label`, `eventType`, `DEFAULT_THEME`, `DEFAULT_THEME_BY_EVENT_TYPE`,
or `themesForEventType()` changed. No new theme was added — Green House and
any other unfinished/external theme remain absent from the registry.

### Tests run

1. **`require('./config/themes')` loads without error** — confirmed; all four
   original exports (`AVAILABLE_THEMES`, `DEFAULT_THEME`,
   `DEFAULT_THEME_BY_EVENT_TYPE`, `themesForEventType`) still present and
   unchanged in shape.
2. **All three themes have a non-empty tagline** — confirmed programmatically
   (non-empty string check on all three).
3. **Every swatch color is a valid hex color** — confirmed programmatically
   (regex check against all 12 values across the three themes; all pass).
4. **Existing fields and helper exports unchanged** — confirmed via
   `git diff config/themes.js` (the diff is purely additive: new fields on
   existing object literals, one new comment block; every previously-existing
   line is untouched) and by re-running `themesForEventType('wedding')` /
   `themesForEventType('birthday')`, both returning the same slugs as before.
5. **Local app started, existing pages confirmed to still render** — with the
   session's `.env` still pointed at the production Neon database (left over
   from the just-completed Phase 2 deployment; confirmed with the person
   before proceeding, since this test is read-only — GET requests only, no
   writes), all 5 existing production events' invitation pages
   (`/invite/1`, `/invite/2`, `/invite/4`, `/invite/8`, `/invite/10`) and
   `/admin/login` returned `200` with no errors in the server log.
   Authenticated admin pages (where `AVAILABLE_THEMES` is actually consumed,
   in the Create/Edit theme dropdowns) could not be exercised this way — the
   local `.env`'s `ADMIN_EMAIL`/`ADMIN_PASSWORD` are dev-only and don't match
   whatever admin account exists in production, and the session store for a
   server pointed at the production DB is the production sessions table.
   This is the same "no production admin credentials" gap already
   established in Phase 1 and Phase 2, not a new limitation. The data
   structure itself was already fully validated by tests 1–4 above, which is
   sufficient to be confident any consumer that iterates
   `AVAILABLE_THEMES` (exactly what the admin dropdowns do) continues to work
   unchanged.

### Known limitation

Same as Phase 1/2: no production admin credentials available this session, so
the one code path that actually reads the new fields' *sibling* data
(`AVAILABLE_THEMES` iteration in the admin Create/Edit theme dropdowns) was
only exercised indirectly (structural validation, not a rendered page). No
